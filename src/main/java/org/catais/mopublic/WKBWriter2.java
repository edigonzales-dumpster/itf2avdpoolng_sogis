...
// Adjusted for Spatialite WKB / Stefan Ziegler
...
public class WKBWriter2
{
	 ...
	 */
	public WKBWriter2(int outputDimension) {
	 ...
	 */
	public WKBWriter2(int outputDimension, int byteOrder) {
	...
	public void write(Geometry geom, OutStream os) throws IOException
	{
		...
		// LinearRings will be written as LineStrings
			...
			writeMultiGeometry(WKBConstants.wkbMultiPoint, (MultiPoint) geom, os);
		else if (geom instanceof MultiLineString)
			writeMultiGeometry(WKBConstants.wkbMultiLineString,
					(MultiLineString) geom, os);
		else if (geom instanceof MultiPolygon)
			writeMultiGeometry(WKBConstants.wkbMultiPolygon,
...
//					(GeometryCollection) geom, os);
	...
	private void writePoint(Point pt, OutStream os) throws IOException
	{
		...
		// SpatiaLite SRID
		ByteOrderValues.putInt(pt.getSRID(), buf, byteOrder);
		os.write(buf, 4);

		// SpatiaLite MBR
		Envelope env = pt.getEnvelopeInternal();
		ByteOrderValues.putDouble(env.getMinX(), buf, byteOrder);
		os.write(buf, 8);
		ByteOrderValues.putDouble(env.getMinY(), buf, byteOrder);
		os.write(buf, 8);
		ByteOrderValues.putDouble(env.getMaxX(), buf, byteOrder);
		os.write(buf, 8);
		ByteOrderValues.putDouble(env.getMaxY(), buf, byteOrder);
		os.write(buf, 8);    
	...
	throws IOException
	{
		...
		ByteOrderValues.putInt(line.getSRID(), buf, byteOrder);
		os.write(buf, 4);
	...
	private void writePolygon(Polygon poly, OutStream os) throws IOException
	{
		...
		ByteOrderValues.putInt(poly.getSRID(), buf, byteOrder);
		os.write(buf, 4);
	...
	private void writeMultiGeometry(int geometryType, GeometryCollection gc, OutStream os) throws IOException {
		byte[] b = new byte[1]; 
		b[0] = (byte) 0x00;
		os.write(b, 1);
		...
		ByteOrderValues.putInt(gc.getSRID(), buf, byteOrder);
		os.write(buf, 4);

		Envelope env = gc.getEnvelopeInternal();
		ByteOrderValues.putDouble(env.getMinX(), buf, byteOrder);
		os.write(buf, 8);
		ByteOrderValues.putDouble(env.getMinY(), buf, byteOrder);
		os.write(buf, 8);
		...
		for (int i = 0; i < gc.getNumGeometries(); i++) {
			...
			Polygon poly = (Polygon) gc.getGeometryN(i);
			writeInt(poly.getNumInteriorRing() + 1, os);
			writeCoordinateSequence(poly.getExteriorRing().getCoordinateSequence(), true, os);
			for (int j = 0; j < poly.getNumInteriorRing(); j++) {
				writeCoordinateSequence(poly.getInteriorRingN(j).getCoordinateSequence(), true, os);
			}			
		}
